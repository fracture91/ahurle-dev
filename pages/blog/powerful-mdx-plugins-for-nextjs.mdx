...
When I display a list of my blog posts, I want to show a little excerpt of the article to give people a taste of what to expect. I also need the excerpt for SEO reasons.
[Here's a plugin that extracts an excerpt](https://github.com/fracture91/ahurle-dev/blob/9c6ba565a521e75624599e8e0a26c688478ee878/config/mdxExcerpt.js):
...
There are lots of solutions for building a Table of Contents out of your markdown headings.  I've used [`doctoc`](https://github.com/thlorenz/doctoc) before, and there's a [`remark-toc`](https://github.com/remarkjs/remark-toc) plugin.  These all have the same problem I mentioned before: they can only mutate the resulting HTML, rather than provide a data structure I can use in JSX-land.  Let's create that data structure!

This plugin depends on `rehype-slug` and `rehype-autolink-headings` to generate anchor tags with `id` attributes next to my headings.  That makes it so a link like `<a href="#some-heading">` will scroll to the right place.
